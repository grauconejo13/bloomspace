  ...
  useEffect(() => {
    ...
    function loadFromLocalStorage() {
      try {
        ...
        const stored = JSON.parse(
          localStorage.getItem("bloomspaceFlowers") || "[]",
        );
        return stored
    ...
    fetchFlowers()
      .then((flowers) => {
        if (!cancelled) {
          setUserFlowers(
            flowers.map((f) => ({ ...f, plantedAt: formatAge(f.plantedAt) })),
          );
        }
      })
      .catch(() => {
    ...
    setUserFlowers((prev) =>
      prev.map((f) =>
        f.id === updatedFlower.id
          ? {
              ...f,
              expiresAt: updatedFlower.expiresAt,
              wateredCount: updatedFlower.wateredCount,
            }
          : f,
      ),
    );
    ...
    >
      ...
      <section className="text-center px-6 pt-20 pb-12">
        ...
        <div className="flex items-center justify-center gap-8 mb-10">
          ...
          <div
            className="w-px h-8"
            style={{ background: "rgba(184, 212, 182, 0.5)" }}
          />
        ...
        <div className="max-w-md mx-auto mb-8 px-6">
          ...
          >
            ...
            <button
              ...
              onMouseEnter={(e) =>
                (e.currentTarget.style.background = "rgba(184,212,182,0.22)")
              }
              onMouseLeave={(e) =>
                (e.currentTarget.style.background = "rgba(255,251,245,0.7)")
              }
            >
              Try again
            </button>
